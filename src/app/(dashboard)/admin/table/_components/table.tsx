...
export default function TableManagement() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (currentSearch) {
        ...
        if (isNumber) {
          query.or(
            `name.ilike.%${currentSearch}%,description.ilike.%${currentSearch}%,status.ilike.%${currentSearch}%,capacity.eq.${currentSearch}`
          );
        }
      }
  ...
  const { data: allTables, refetch: refetchAllTables } = useQuery({
    queryKey: ["all-tables"],
    queryFn: async () => {
      ...
      <Tabs defaultValue="list">
        ...
        <TabsContent value="list">
          <div className="flex gap-2 justify-between mb-4">
            <Input placeholder="Search..." className="max-w-64" onChange={(e) => handleChangeSearch(e.target.value)} />
            <Dialog>
