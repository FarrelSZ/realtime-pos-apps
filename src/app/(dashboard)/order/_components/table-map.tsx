    ...
    <HoverCard>
      ...
      <HoverCardContent className="mt-2">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-muted-foreground">Capacity: {data.capacity}</p>
          <p className="text-xs text-muted-foreground">Status: {data.status}</p>
          {data.order ? (
            <div className="flex flex-col gap-2">
              <p className="text-xs text-muted-foreground">Order ID : {data.order.order_id}</p>
              <p className="text-xs text-muted-foreground">Customer : {data.order.customer_name}</p>
                ...
                <Fragment>
                  {profile.role !== "kitchen" && (
                    <div className="w-full flex gap-2">
                      <Button
                        variant="destructive"
                        onClick={() => data.handleReservation(`${data?.order?.id}`, data.id, "canceled")}
                      >
                        Canceled
                      </Button>
...
export default function TableMap({
  ...
  return (
    <div className="w-[100%] h-[80vh] border rounded-lg">
      <ReactFlow proOptions={{ hideAttribution: true }} nodes={initialNodes} nodeTypes={nodeTypes}>
        <Background />
      </ReactFlow>
    </div>
  );
